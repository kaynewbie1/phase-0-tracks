...
#Release 2: Analyze state_data
  #What is going on with this hash?
    - It has additional hashes embedded in a master hash
  #What does it have in it? (HINT: There are two different syntax used for hashes here. Key value pairs inside of key value pairs.)
    - The value hash uses a colon instead of a hashrocket to define its key value pairs.)
  #What's the difference?) I refer to this giant hash as a "hashy hash;" why do you think that is?
    - becayse the value for the main key value pair for the constant STATE_DATA has a hash for it value
